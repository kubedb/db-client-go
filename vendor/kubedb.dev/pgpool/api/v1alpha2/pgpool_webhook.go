...
func (p *Pgpool) ValidateCreateOrUpdate() field.ErrorList {
	...
	if p.Spec.PodTemplate != nil {
		...
		err = validateVolumesMountPaths(p.Spec.PodTemplate)
		if err != nil {
...
func validateVersion(p *Pgpool) error {
...
func validateVolumes(p *Pgpool) error {
	if p.Spec.PodTemplate.Spec.Volumes == nil {
		return nil
	}

	for _, rv := range reservedVolumes {
...
func validateVolumesMountPaths(podTemplate *ofst.PodTemplateSpec) error {
	if podTemplate == nil {
		return nil
	}
	...
	for _, rvmp := range reservedVolumesMountPaths {
...
var reservedVolumesMountPaths = []string{
