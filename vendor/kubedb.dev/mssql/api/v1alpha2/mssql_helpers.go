...
func (m *MsSQL) Owner() *meta.OwnerReference {
	return meta.NewControllerRef(m, apm.SchemeGroupVersion.WithKind(m.ResourceKind()))
}
...
func (m *MsSQL) ServiceLabels(alias apm.ServiceAlias, extraLabels ...map[string]string) map[string]string {
	svcTemplate := apm.GetServiceTemplate(m.Spec.ServiceTemplates, alias)
