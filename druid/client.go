...
func (c *Client) GetDruidDatasources() ([]DatasourceInfo, error) {
	...
	path := "druid/coordinator/v1/datasources?simple"

	// Declare a slice to hold the response
	var datasources []DatasourceInfo

	// Execute the HTTP GET request
	_, err := c.ExecuteRequest(http.MethodGet, path, nil, &datasources)
	if err != nil {
		klog.Error("Failed to execute GET request for Druid datasources: ", err)
		return nil, err
	}
